...
class RoadGenerator():
    # 初期化
    def __init__(self, area, heightmap, new_array, area_with_border):
        ...
        for one_p in path_list:
            ...
            for x_w in range(x-self.width//2, x + self.width//2+1):
                if x_w < 0 or x_w > self.heightmap.shape[0] or x_w == x:
                    continue
                if i % 4 == 1 or i % 4 == 2:
                    ED.placeBlock((x_w + self.area[0], self.heightmap[x_w][z]-1, r_z), Block('sea_lantern'))
                else:
                    ED.placeBlock((x_w + self.area[0], self.heightmap[x_w][z]-1, r_z), Block('stone_bricks'))
            for z_w in range(z-self.width//2, z + self.width//2+1):
                if z_w < 0 or z_w > self.heightmap.shape[1] or z_w == z:
                    continue
                if i % 4 == 1 or i % 4 == 2:
                    ED.placeBlock((r_x, self.heightmap[x][z_w]-1, z_w + self.area[1]), Block('sea_lantern'))
                else:
                    ED.placeBlock((r_x, self.heightmap[x][z_w]-1, z_w + self.area[1]), Block('stone_bricks'))
            if i%4==1:
                ED.placeBlock((r_x, self.heightmap[one_p[0]][one_p[1]]-1, r_z), Block('sea_lantern'))
            else:
                ED.placeBlock((r_x, self.heightmap[one_p[0]][one_p[1]]-1, r_z), Block('stone_bricks'))
            i += 1
